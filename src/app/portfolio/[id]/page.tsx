...
export default function ProjectDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  // Compatible with Next.js 14, 15 and 16 params format
  ...
  useEffect(() => {
    ...
    fetch("/api/content")
      .then((res) => res.json())
      .then((data) => {
        const item = data.showcaseItems?.find((s: any) => s.id === id);
        if (item) {
          setProject(item);
        }
      })
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0b0a0a] text-[#d9bb97] flex flex-col justify-between">
        <Header />
        <div className="flex-1 flex flex-col items-center justify-center pt-24">
          <div className="w-8 h-8 border-2 border-[#b34a26] border-t-transparent rounded-full animate-spin mb-4" />
          <p className="text-sm text-[rgba(217,187,151,0.5)]">Retrieving project archives...</p>
        </div>
        <Footer />
      </div>
    );
  ...
  // Combined list of assets for masonry display: Cover Image + Grouped Images
  ...
  return (
    <div className="relative overflow-x-hidden min-h-screen bg-[#0b0a0a] text-[#d9bb97]">
      ...
      <div className="absolute top-0 left-1/4 w-[500px] h-[500px] bg-[#b34a26] rounded-full blur-[180px] opacity-10 pointer-events-none" />
      ...
      <main className="max-w-7xl mx-auto px-6 pt-32 pb-24 relative z-10">
        ...
        {/* Masonry Grid of Portfolio Assets */}
        <div>
          ...
          <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6">
            {allAssets.map((url, idx) => (
              <motion.div
                ...
                className="group relative cursor-pointer overflow-hidden rounded-2xl border border-[rgba(217,187,151,0.08)] bg-black/20 hover:border-[#b34a26]/40 transition-all break-inside-avoid"
              >
        ...
        {/* Project External redirect footer */}
